...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--back_months", required=True, help="Back months for training data (e.g. 1,2,3).")
    p.add_argument(
        "--data-path",
        required=False,
...
def prepare_training_subset(
    df: pd.DataFrame, test_dts_val: pd.Timestamp, back_month: int
    ...
    """
    train_start_val = test_dts_val - relativedelta(months=back_month)
...
def main():
    ...
    back_months = int(args.back_months)
    ...
    train_df, train_start_val, train_end_exclusive = prepare_training_subset(df, test_dts_val, back_months)
